...
function update(event) {
    try {
        ...
        const pathToEvent = path.join(pathToDao, 'storage', 'events', `${event.id}.json`);
...
function create(event) {
    try {
        ...
        const pathToNewEvent = path.join(pathToDao, 'storage', 'events', `${event.id}.json`)
...
function list() {
    try {
        ...
        // list of names of all files in directory
...
function getEventGroups(listGroupIDs) {
    ...
    const filteredGroups = allGroups.filter(group => listGroupIDs.includes(group.id))
